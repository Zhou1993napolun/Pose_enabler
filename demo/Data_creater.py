...
def main():
    ...
    for dir_name in result_txt_name:
        data_img_num_list.append(dircount('./new_dataset/'+dir_name))
        print(dircount('./new_dataset/'+dir_name))
    ...
    if args.webcam or args.video:
        ...
        for num_result in range(len(result_txt_name)):
            pred_boxes = []
            loss_loc = []
            for i in range(data_img_num_list[num_result]):
                data_img_list = []
                img_bgr = cv2.imread('./new_dataset/'+result_txt_name[num_result]+'/{}.jpg'.format(i))
            ...
            if len(pred_boxes) >= 1:
                ...
                for box in pred_boxes:
                    if box != []:
                        center, scale = box_to_center_scale(box, cfg.MODEL.IMAGE_SIZE[0], cfg.MODEL.IMAGE_SIZE[1])
                        img_bgr = cv2.imread('./new_dataset/'+result_txt_name[num_result]+'/{}.jpg'.format(loss_loc[num_cnt]))
                        img_rgb = img_bgr[:, :, [2, 1, 0]]
                ...
                print(result.shape)
                np.savetxt("./new_dataset/" + result_txt_name[num_result] + "_0.txt", result_0)
                np.savetxt("./new_dataset/" + result_txt_name[num_result] + "_1.txt", result_1)
            else:
